    ...
    function construirLayoutBase() {
        app.innerHTML = `
            <div class="min-h-screen bg-fundo relative font-sans text-texto select-none overflow-x-hidden">
                ...
                <main id="conteudo-principal" class="w-full min-h-screen pt-28 pb-32 px-6 flex flex-col items-center justify-center relative z-10"></main>

                <nav class="fixed bottom-6 left-0 w-full z-50 px-6 pointer-events-none">
                    <div id="tab-bar-container" class="pointer-events-auto max-w-[400px] mx-auto flex justify-between items-end bg-transparent"></div>
                </nav>
        ...
        container.innerHTML = abas.map(aba => {
            ...
            return `
                <button data-aba="${aba.id}" class="flex flex-col items-center justify-end w-14 h-14 outline-none select-none group">
                    ...
                    ${isAtiva ? `<span class="text-[10px] font-bold text-ouro tracking-wide mt-1 animate-[fadeIn_0.3s_ease-in-out]">${aba.label}</span>` : ''}
                </button>
            `;
        }).join('');
